...
After you enable a payment service in [!INCLUDE[navnow](includes/navnow_md.md)], a link to the service is available on sales documents that you send by email to your customers. Customers can use the link to go to the payment service and pay the bill, directly from the sales document. If you don't want to include the link, for example, if a customer will pay with cash, you can remove the payment service from the invoice before posting.  

The Microsoft Pay, PayPal Payments Standard, and WorldPay Payments Standard extensions are installed in [!INCLUDE[navnow](includes/navnow_md.md)], and are ready for you to enable.  

## To enable a payment service in [!INCLUDE[navnow](includes/navnow_md.md)]
