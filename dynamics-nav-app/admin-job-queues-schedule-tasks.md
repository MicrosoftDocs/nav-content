...
[!INCLUDE[navnow](includes/navnow_md.md)] automatically runs the jobs according to the specified schedules for each job queue entry. You can also start, stop, and put a job queue entry on hold manually.
...
Job queues are an effective tool to schedule the running of business processes in the background. For example, there may be an instance in which multiple users are trying to post sales orders at the same time, but only one order can be processed at a time. By setting up a background posting routine, you can place the postings in a queue for processing in the background.  
 ...
 [!INCLUDE[navnow](includes/navnow_md.md)] supports background posting for the following document types:  
...
> [!IMPORTANT]  
>  If you use the SUPER permissions set that comes with [!INCLUDE[navnow](includes/navnow_md.md)], you and your users have permissions to run all objects. In this case, access for each user is only limited by permissions for data.  
